...
public class Pawn {
	...
	public static boolean pawnMoveValid(int piecePosition, int movePosition) {
		...
		if (MyGame.currentPlayer == MyGame.WHITE) {
			...
			return pawnMoveValid(piecePosition, movePosition, forward,
					rDiagonal, lDiagonal, enemyColor);
		}
	...
	private static boolean pawnMoveValid(int piecePosition, int movePosition,
		...
		if (piecePosition == (movePosition + forward)) { // move straight one
															// step
			if (MyGame.board[movePosition] == MyGame.SQUARE) {
				...
				return false;
		} else if (piecePosition == (movePosition + rDiagonal)) { // move right
																	// diagonal
																	// only if
																	// there is
																	// an enemy
			if (MyGame.board[movePosition] / 10 == enemyColor) { // there is an
																	// enemy
				return true;
			} else
				return false;
		} else if (piecePosition == (movePosition + lDiagonal)) { // move left
																	// diagonal
																	// only if
																	// there is
																	// an enemy
			if (MyGame.board[movePosition] / 10 == enemyColor)
				...
				return false;
		} else if (((piecePosition / 10 == 2 && MyGame.currentPlayer == MyGame.BLACK) || (piecePosition / 10 == 7
				&& MyGame.currentPlayer == MyGame.WHITE))
				&& piecePosition == (movePosition + 2 * forward)) {
			if (MyGame.board[movePosition] == MyGame.SQUARE) {
				return true;
			} else
				return false;
